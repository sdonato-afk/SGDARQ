  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const [sContratos, sObras, sProps, sVenc] = await Promise.all([
        getDocs(collection(db, ...MAIN_COL('contratos'))),
        getDocs(query(collection(db, ...MAIN_COL('obras')), where('estado', '==', 'En Proceso'))),
        getDocs(collection(db, ...MAIN_COL('propiedades'))),
        getDocs(collection(db, ...MAIN_COL('vencimientos'))),
      ]);
