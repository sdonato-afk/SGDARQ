...
import { db, EVENTOS_COL } from '../config/firebase';
...
export function useEventosData() {
  ...
  const agregarEvento = async (data) => {
    await addDoc(collection(db, ...EVENTOS_COL), {
      ...data,
      completado: false,
  ...
  const completarEvento = async (id) => {
    ...
    // Marcar completado
    await updateDoc(doc(db, ...EVENTOS_COL, id), { completado: true });
  ...
  const eliminarEvento = async (id) => {
    await deleteDoc(doc(db, ...EVENTOS_COL, id));
  };
