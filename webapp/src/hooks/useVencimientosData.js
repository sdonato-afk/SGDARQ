...
export function useVencimientosData() {
  ...
  useEffect(() => {
    const ref = collection(db, 'artifacts', appId, 'public', 'data', 'vencimientos');
    ...
    const refInbox = query(collection(db, 'artifacts', appId, 'public', 'data', 'inbox_movimientos'), where('estado', '==', 'pendiente'));
  ...
  useEffect(() => {
    ...
    getDocs(query(
      ...
      // Filtrar 'Egreso' en cliente para evitar requerir index compuesto (tipo + fecha) en Firestore
  ...
  const agregarVencimiento = async (item) => {
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'vencimientos'), {
      ...item,
  ...
  const actualizarVencimiento = async (id, data) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'vencimientos', id), {
      ...data,
  ...
  const marcarPagado = async (id, item) => {
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'vencimientos', id), { pagado: true });
  ...
  const eliminarVencimiento = async (id) => {
    await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'vencimientos', id));
  };
